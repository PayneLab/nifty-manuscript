...
# input files
quant_file = os.path.join(script_directory, os.pardir, os.pardir, 'Data_Prep', 'Ai_Van-Eyk_2025', 'NIFty_Ready_Data', 'quant_imputed.tsv')
...
main_output_dir = os.path.join(script_directory, 'Test_Ai_Imputed')
...
for split in splits:
    for i in range(tests):
        ...
        validate_samples = sorted(random.sample(sorted(day0_samples), num_samples_per_class_validation) + random.sample(sorted(day21_samples), num_samples_per_class_validation))
        ...
        fs_samples = sorted(random.sample(sorted(day0_samples_updated), num_FS_samples) + random.sample(sorted(day21_samples_updated), num_FS_samples))
        ...
        train_test_samples = sorted(random.sample(sorted(day0_samples_updated), num_train_test_samples) + random.sample(sorted(day21_samples_updated), num_train_test_samples))
