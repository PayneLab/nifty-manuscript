...
# input files
quant_file = os.path.join(script_directory, os.pardir, os.pardir, 'Data_Prep', 'Furtwaengler_Porse_Schoof_2025', 'NIFty_Ready_Data', 'quant_unimputed.tsv')
meta_file = os.path.join(script_directory, os.pardir, os.pardir, 'Data_Prep', 'Furtwaengler_Porse_Schoof_2025', 'NIFty_Ready_Data', 'meta.tsv')

quant_table = pd.read_csv(quant_file, sep='\t')
meta_table = pd.read_csv(meta_file, sep='\t')

HSC_samples = set(meta_table[meta_table['classification_label'] == "HSC"]['sample_id'].tolist())
EE_samples = set(meta_table[meta_table['classification_label'] == "Early Eryth"]['sample_id'].tolist())
...
main_output_dir = os.path.join(script_directory, 'Test_Furtwaengler_Unimputed_HSCxEarlyEryth')
...
for split in splits:
    for i in range(tests):
        ...
        identifier = f"Furtwaengler_Unimputed_HSCxEE_Split{split}_Test{test + 1}"
        HSC_samples_updated = HSC_samples.copy()
        EE_samples_updated = EE_samples.copy()

        # create the validation data
        validate_samples = sorted(random.sample(sorted(HSC_samples), num_samples_per_class_validation) + random.sample(sorted(EE_samples), num_samples_per_class_validation))
        HSC_samples_updated.difference_update(validate_samples)
        EE_samples_updated.difference_update(validate_samples)
        ...
        fs_samples = sorted(random.sample(sorted(HSC_samples_updated), num_FS_samples) + random.sample(sorted(EE_samples_updated), num_FS_samples))
        HSC_samples_updated.difference_update(fs_samples)
        EE_samples_updated.difference_update(fs_samples)
        ...
        train_test_samples = sorted(random.sample(sorted(HSC_samples_updated), num_train_test_samples) + random.sample(sorted(EE_samples_updated), num_train_test_samples))
        HSC_samples_updated.difference_update(train_test_samples)
        EE_samples_updated.difference_update(train_test_samples)
