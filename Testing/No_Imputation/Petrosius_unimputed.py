...
# input files
quant_file = os.path.join(script_directory, os.pardir, os.pardir, 'Data_Prep', 'Petrosius_Schoof_2025', 'NIFty_Ready_Data', 'quant_unimputed.tsv')
meta_file = os.path.join(script_directory, os.pardir, os.pardir, 'Data_Prep', 'Petrosius_Schoof_2025', 'NIFty_Ready_Data', 'meta.tsv')

quant_table = pd.read_csv(quant_file, sep='\t')
meta_table = pd.read_csv(meta_file, sep='\t')

HEK_samples = set(meta_table[meta_table['classification_label'] == "HEK"]['sample_id'].tolist())
u937_samples = set(meta_table[meta_table['classification_label'] == "u937"]['sample_id'].tolist())
...
main_output_dir = os.path.join(script_directory, 'Test_Petrosius_Unimputed')
...
for split in splits:
    for i in range(tests):
        ...
        identifier = f"Petrosius_Unimputed_Split{split}_Test{test + 1}"
        HEK_samples_updated = HEK_samples.copy()
        u937_samples_updated = u937_samples.copy()

        # create the validation data
        validate_samples = sorted(random.sample(sorted(HEK_samples), num_samples_per_class_validation) + random.sample(sorted(u937_samples), num_samples_per_class_validation))
        HEK_samples_updated.difference_update(validate_samples)
        u937_samples_updated.difference_update(validate_samples)
        ...
        fs_samples = sorted(random.sample(sorted(HEK_samples_updated), num_FS_samples) + random.sample(sorted(u937_samples_updated), num_FS_samples))
        HEK_samples_updated.difference_update(fs_samples)
        u937_samples_updated.difference_update(fs_samples)
        ...
        train_test_samples = sorted(random.sample(sorted(HEK_samples_updated), num_train_test_samples) + random.sample(sorted(u937_samples_updated), num_train_test_samples))
        HEK_samples_updated.difference_update(train_test_samples)
        u937_samples_updated.difference_update(train_test_samples)
