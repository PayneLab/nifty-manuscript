...
# input files
quant_file = os.path.join(script_directory, os.pardir, os.pardir, 'Data_Prep', 'Khan_Elcheikhali_Slavov_2024', 'NIFty_Ready_Data', 'quant_unimputed_mfn.tsv')
quant_file = os.path.join(script_directory, os.pardir, os.pardir, 'Data_Prep', 'Khan_Elcheikhali_Slavov_2024', 'NIFty_Ready_Data', 'meta_mfn.tsv')

quant_table = pd.read_csv(quant_file, sep='\t')
meta_table = pd.read_csv(meta_file, sep='\t')

smc_samples = set(meta_table[meta_table['classification_label'] == "smc"]['sample_id'].tolist())
fibro_samples = set(meta_table[meta_table['classification_label'] == "fibro"]['sample_id'].tolist())
...
main_output_dir = os.path.join(script_directory, 'Test_Saddic_Unimputed_MFN')
...
for split in splits:
    for i in range(tests):
        ...
        identifier = f"Saddic_Unimputed_MFN_Split{split}_Test{test + 1}"
        smc_samples_updated = smc_samples.copy()
        fibro_samples_updated = fibro_samples.copy()

        # create the validation data
        validate_samples = sorted(random.sample(sorted(smc_samples), num_samples_per_class_validation) + random.sample(sorted(fibro_samples), num_samples_per_class_validation))
        smc_samples_updated.difference_update(validate_samples)
        fibro_samples_updated.difference_update(validate_samples)
        ...
        fs_samples = sorted(random.sample(sorted(smc_samples_updated), num_FS_samples) + random.sample(sorted(fibro_samples_updated), num_FS_samples))
        smc_samples_updated.difference_update(fs_samples)
        fibro_samples_updated.difference_update(fs_samples)
        ...
        train_test_samples = sorted(random.sample(sorted(smc_samples_updated), num_train_test_samples) + random.sample(sorted(fibro_samples_updated), num_train_test_samples))
        smc_samples_updated.difference_update(train_test_samples)
        fibro_samples_updated.difference_update(train_test_samples)
