...
# input files
quant_file = os.path.join(script_directory, os.pardir, os.pardir, 'Data_Prep', 'Leduc_Slavov_2022', 'NIFty_Ready_Data', 'quant_unimputed.tsv')
meta_file = os.path.join(script_directory, os.pardir, os.pardir, 'Data_Prep', 'Leduc_Slavov_2022', 'NIFty_Ready_Data', 'meta.tsv')

quant_table = pd.read_csv(quant_file, sep='\t')
meta_table = pd.read_csv(meta_file, sep='\t')

m_samples = set(meta_table[meta_table['classification_label'] == "m"]['sample_id'].tolist())
u_samples = set(meta_table[meta_table['classification_label'] == "u"]['sample_id'].tolist())


# set up testing information
splits = [300, 25, 30, 40, 50, 75, 100, 200]  # num samples per class to start with and be split into train test
tests = 100
fs_ratio = 0.3
...
main_output_dir = os.path.join(script_directory, 'Test_Leduc_Unimputed')
...
for split in splits:
    for i in range(tests):
        ...
        identifier = f"Leduc_Unimputed_Split{split}_Test{test + 1}"
        m_samples_updated = m_samples.copy()
        u_samples_updated = u_samples.copy()

        # create the validation data
        validate_samples = sorted(random.sample(sorted(m_samples), num_samples_per_class_validation) + random.sample(sorted(u_samples), num_samples_per_class_validation))
        m_samples_updated.difference_update(validate_samples)
        u_samples_updated.difference_update(validate_samples)
        ...
        fs_samples = sorted(random.sample(sorted(m_samples_updated), num_FS_samples) + random.sample(sorted(u_samples_updated), num_FS_samples))
        m_samples_updated.difference_update(fs_samples)
        u_samples_updated.difference_update(fs_samples)
        ...
        train_test_samples = sorted(random.sample(sorted(m_samples_updated), num_train_test_samples) + random.sample(sorted(u_samples_updated), num_train_test_samples))
        m_samples_updated.difference_update(train_test_samples)
        u_samples_updated.difference_update(train_test_samples)
