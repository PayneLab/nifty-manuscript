...
# input files
quant_file = os.path.join(script_directory, os.pardir, os.pardir, 'Data_Prep', 'Montalvo_Alvarez-Dominguez_Slavov_2023', 'NIFty_Ready_Data', 'quant_unimputed.tsv')
meta_file = os.path.join(script_directory, os.pardir, os.pardir, 'Data_Prep', 'Montalvo_Alvarez-Dominguez_Slavov_2023', 'NIFty_Ready_Data', 'meta.tsv')

quant_table = pd.read_csv(quant_file, sep='\t')
meta_table = pd.read_csv(meta_file, sep='\t')

ko_samples = set(meta_table[meta_table['classification_label'] == "ko"]['sample_id'].tolist())
...
main_output_dir = os.path.join(script_directory, 'Test_Montalvo_Unimputed')
...
for split in splits:
    for i in range(tests):
        ...
        identifier = f"Montalvo_Unimputed_Split{split}_Test{test + 1}"
        ko_samples_updated = ko_samples.copy()
        wt_samples_updated = wt_samples.copy()

        # create the validation data
        validate_samples = sorted(random.sample(sorted(ko_samples), num_samples_per_class_validation) + random.sample(sorted(wt_samples), num_samples_per_class_validation))
        ko_samples_updated.difference_update(validate_samples)
        ...
        fs_samples = sorted(random.sample(sorted(ko_samples_updated), num_FS_samples) + random.sample(sorted(wt_samples_updated), num_FS_samples))
        ko_samples_updated.difference_update(fs_samples)
        ...
        train_test_samples = sorted(random.sample(sorted(ko_samples_updated), num_train_test_samples) + random.sample(sorted(wt_samples_updated), num_train_test_samples))
        ko_samples_updated.difference_update(train_test_samples)
