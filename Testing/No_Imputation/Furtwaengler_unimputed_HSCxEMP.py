...
EMP_samples = set(meta_table[meta_table['classification_label'] == "EMP"]['sample_id'].tolist())
...
main_output_dir = os.path.join(script_directory, 'Test_Furtwaengler_Unimputed_HSCxEMP')
...
for split in splits:
    for i in range(tests):
        ...
        identifier = f"Furtwaengler_Unimputed_HSCxEMP_Split{split}_Test{test + 1}"
        HSC_samples_updated = HSC_samples.copy()
        EMP_samples_updated = EMP_samples.copy()

        # create the validation data
        validate_samples = sorted(random.sample(sorted(HSC_samples), num_samples_per_class_validation) + random.sample(sorted(EMP_samples), num_samples_per_class_validation))
        HSC_samples_updated.difference_update(validate_samples)
        EMP_samples_updated.difference_update(validate_samples)
        ...
        fs_samples = sorted(random.sample(sorted(HSC_samples_updated), num_FS_samples) + random.sample(sorted(EMP_samples_updated), num_FS_samples))
        HSC_samples_updated.difference_update(fs_samples)
        EMP_samples_updated.difference_update(fs_samples)
        ...
        train_test_samples = sorted(random.sample(sorted(HSC_samples_updated), num_train_test_samples) + random.sample(sorted(EMP_samples_updated), num_train_test_samples))
        HSC_samples_updated.difference_update(train_test_samples)
        EMP_samples_updated.difference_update(train_test_samples)
